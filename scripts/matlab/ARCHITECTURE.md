...
> 最后更新：A.4.0 完成后（纯符号化管线，移除 numeric/symbolic 双轨）。
...
```mermaid
flowchart TD
    A["完整的连接图（含闭环）"] --> B["closed: false<br/>生成树边 (spanning-tree edge)<br/>g.addMate()"]
    A --> C["closed: true<br/>弦边 (chord edge)<br/>g.addClosedMate()"]
    B --> B1["双向插入 (socket↔plug)"]
    B --> B2["参与 FK 位姿传播"]
    B --> B3["定义机构主干位姿流向"]
    C --> C1["单向插入 (socket→plug)"]
    C --> C2["不参与 FK 位姿传播"]
    C --> C3["仅用于诊断闭环残差"]
```
...
`PosePropagator.propagatePoses` 的迭代传播逻辑为：
...
位姿传播的起点是**根节点（root node）**，由 `Expander` 在构造函数末尾处理：

```matlab
% Expander constructor, after all instances + connections processed:
if ~obj.EdgeGraph_.hasRootNodes()
    obj.EdgeGraph_.addRoot(obj.Instances(1).bodies{1}.node);  % fallback
end
obj.Poses = obj.EdgeGraph_.propagate();
```

**根节点来源优先级**：
1. DSL 中标记了 `semantic_tag: root` 的 frame → 自动注册为 root node（`semantic_tag: ground` 仅标识 L3 世界绑定端点，不触发 root 注册）
2. 无 root node 时 → 回退到第一个实例的第一个 body 作为根
3. 支持多根（多次调用 `addRoot()`），适用于多分支 / 并联机构

**`g.propagate()` 执行流程**：

```mermaid
flowchart TD
    A["g.propagate()"] --> B["1. 构造 seed map<br/>每个 root node → eye(4)<br/>（世界原点位姿）<br/>无 root node 时<br/>→ Edges(1).from → eye(4) 作为 fallback"]
    A --> C["2. g.toStruct()<br/>过滤 closed_mate 边 + 剥离 kind<br/>→ 纯 from/to/T struct 数组"]
    A --> D["3. PosePropagator.propagatePoses(edges, seed)"]
    D --> E["返回 containers.Map<br/>(name → 4×4 homogeneous transform)"]
```
...
FK 传播后，某些 frame 可能不出现在 `poses` map 中——这表示它们属于**不连通分量**（disconnected component），在图结构中无法从任何 root node 通过边到达：
...
```mermaid
flowchart TD
    A["specs/dsl/examples/*.yaml<br/>← 机构装配 DSL"] --> B["core.readYaml()"]
    B --> C["DSL struct<br/>(mechanism / instances / connections / closed flags)"]
    C --> D["localExpandInstance()<br/>遍历 instances，加载 module def YAML，注入参数"]
    D --> D1["g.addFixedTransform(from, to, T)<br/>固定变换（双向）"]
    D --> D2["g.addJoint(from, to, axis, val, kind)<br/>关节（双向，含关节变量）"]
    D --> D3["g.addRoot(node)<br/>标记根帧"]
    D1 & D2 & D3 --> E["遍历 connections，匹配 socket↔plug"]
    subgraph conn["连接分流"]
        E1["closed: false → g.addMate()<br/>生成树边（双向）"]
        E2["closed: true → g.addClosedMate()<br/>弦边（单向，诊断用）"]
    end
    E --> E1
    E --> E2
    E1 & E2 --> F["g.propagate()"]
    F --> F1["构造 seed map (root → eye(4))"]
    F --> F2["g.toStruct() → 纯 from/to/T 数组"]
    F --> F3["PosePropagator.propagatePoses()<br/>→ poses: containers.Map"]
    F1 & F2 & F3 --> G["可视化循环"]
    G --> G1["poses(f.node) → triad + geometry patch"]
    G --> G2["mate gap / Zdot → 诊断线段 + 控制台报告"]
    G --> G3["unplaced 列表 → 警告不连通分量"]
```

此流水线（由 `ir.Expander` 编排）将声明式的 YAML DSL 描述完整转化为可交互的 3D 可视化，同时提供闭环残差诊断，帮助验证机构设计的几何正确性。

符号 FK 路径复用同一套 `Expander → EdgeGraph → PosePropagator` 管线（A.4.0 纯符号化，始终 `sym`），由 `ir.TaskFrame` 在出口处将符号位姿 map 转化为可分解、可代入的符号表达式。数值代入可通过 `fk.eval(vals)` 或 `e.evaluateNumeric(configYaml)` 完成。
