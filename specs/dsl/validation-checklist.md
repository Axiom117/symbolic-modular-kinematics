...
| S6 | 实例 `parameters` 块不出现在 v0 DSL 中（v0 禁止 variant，参数值由 `specs/modules/config/dimensions.yaml` 按 module_type 注入） | `instance.additionalProperties: false` + 无 `parameters` |
...
| `single-closed-loop/robot_description.yaml` | 通过 | 通过 | 1 | 1（平行四边形环） |
...
check-jsonschema --schema-file specs/schema/mechanism-assembly.schema.yaml \
  specs/dsl/cases/open-chain-2r/robot_description.yaml \
  specs/dsl/cases/single-closed-loop/robot_description.yaml
```
