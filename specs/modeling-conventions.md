...
>
> **`observable` 与 `exposed` 正交**：`exposed` 控制机械连接合法性（能不能 mate），`observable` 控制解释器输出范围（能不能读）。一个 frame 可以同时是 port 且被观测（如 `Manipulator.dock`），也可以仅是观测点而非 port（如 `Pipette_body.tip_origin`），也可以仅是 port（如大多数 `Frame.faceX+`——位姿随 FK 链自然可得，无需显式标记）。两者独立。详见图 1（§3.6）。
...
| `false` | `true` | 内部观测点：不可连接，但 world 位姿对外输出 | `Pipette_body.tip_origin` |
...
| L1 模块 | 模块类型名 | 首字母大写（UpperCamelCase） | `Frame`、`Pin`、`Joint`、`Adaptor`、`Pipette_body` |
...
- **模块名**首字母大写；库内现存模块保留 SLX 原名（含下划线，如 `Pipette_body`），后续新增模块须使用纯 UpperCamelCase（无下划线）。
