...
>
> **`observable` 与 `exposed` 正交**：`exposed` 控制机械连接合法性（能不能 mate），`observable` 控制解释器输出范围（能不能读）。一个 frame 可以同时是 port 且被观测（如 `Manipulator.dock`），也可以仅是观测点而非 port（如 `ToolPipette.tip_origin`），也可以仅是 port（如大多数 `Frame.faceXPlus`——位姿随 FK 链自然可得，无需显式标记）。两者独立。详见图 1（§3.6）。
...
|-----------|-------------|------|---------|
| `true` | `false`（默认） | 机械端口，可被 mate 连接；位姿随 FK 链自然可得 | `Frame.faceZPlus`、`Adaptor.mount_point` |
...
|------|------|------|------|------|------|
| `Frame` | 结构（0 DOF） | `cubeLength` | `faceXPlus`、`faceXMinus`、`faceYPlus`、`faceYMinus`、`faceZPlus`、`faceZMinus` | 六面均 `socket` | `frame_hyper_cube.STEP` |
...
|------|------|------|------|
| `faceXPlus` | `[cubeLength/2, 0, 0]` | `+X` | `+Z` |
| `faceXMinus` | `[-cubeLength/2, 0, 0]` | `-X` | `+Z` |
| `faceYPlus` | `[0, cubeLength/2, 0]` | `+Y` | `+Z` |
| `faceYMinus` | `[0, -cubeLength/2, 0]` | `-Y` | `+Z` |
| `faceZPlus` | `[0, 0, cubeLength/2]` | `+Z` | `+X` |
| `faceZMinus` | `[0, 0, -cubeLength/2]` | `-Z` | `+X` |
