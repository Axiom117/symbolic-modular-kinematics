...
| `semantic_tag` | 枚举 | 否 | 仅注释，不影响运动学（见 §9.3） |
...
| `variable` | 符号名 | 是 | revolute 用 `q`、prismatic 用 `d`（见 §11） |
...
本节列出当前模块库全部五个核心模块（来源：`slx_module_reference/module_library_reference.md`），作为 A.1 模块 schema 的起点参考。本节只作清单式摘要；每个模块的完整字段 schema、端口变换精确数值与内部运动学图均在 A.1 阶段正式冻结。
...
|------|------|------|------|
| `faceX+` | `[cubeLength/2, 0, 0]` | `+X → +Y` | `+Y → +Z` |
| `faceX-` | `[-cubeLength/2, 0, 0]` | `+X → +Y` | `+Y → +Z` |
| `faceY+` | `[0, cubeLength/2, 0]` | `+X → +Y` | `+Y → +Z` |
| `faceY-` | `[0, -cubeLength/2, 0]` | `+X → +Y` | `+Y → +Z` |
| `faceZ+` | `[0, 0, cubeLength/2]` | `+Y → -Z` | `+Z → +X` |
| `faceZ-` | `[0, 0, -cubeLength/2]` | `+Z → +X` | `+Y → +Z` |
...
- 主轴：端口 **`+Z` 统一为朝模块外的对插法向**（所有 port 一致，不再靠一头朝外、一头朝内的约定凑合）。两端口对接时的面对面翻转由解释器统一施加（见 §10）。
- 位姿：相对宿主中心 `Frame` 的平移与朝向，由一条 `FixedTransform` 给出，不在端口本体声明。

> **并非所有 `exposed` frame 都是机械对接口**。任务/工具参考系（如 `Pipette_body.tip_origin`，`semantic_tag: tool`）虽然 `exposed=true`，但只作 FK/IK 输出接口，不带极性（§9.5），不参与机械连接。
...
| 极性 | 现实对应 | 归属 |
|------|------|------|
| `socket` | 凹面 + 不锈钢贴片（凹陷接收面） | **仅 `Frame` 的六个面端口** |
| `plug` | 凸面 + 磁铁（凸出对插面） | 所有贴到 `Frame` 上的机械面（`Pin`、`Joint`、`Adaptor`、工具连接面等） |

规则：

- **`Frame` 是唯一提供 `socket` 的模块**；其余一切机械对接面均为 `plug`。这与体素架构一致：`Frame` 是体素枢纽（hub），其他模块都插到 `Frame` 面上。
- 由此自然得到合法性：`socket↔plug` 合法；`socket↔socket`（frame–frame）、`plug↔plug`（pin–joint、pin–pin、joint–joint）一律非法。
- `polarity` **可选**：只有机械对接面才声明。任务/工具参考系（如 `tip_origin`）不带 `polarity`，不能作为连接端（§9.1）。
