...
% symbolic Poses; JointVarMap tracks joint variables; KinematicModel extracts
...
assert(isequal(size(km.PosExpr), [3 1]), 'PosExpr must be 3×1.');
assert(isequal(size(km.RotExpr), [3 3]), 'RotExpr must be 3×3.');
assert(isa(km.PosExpr, 'sym'), 'PosExpr must be sym.');
assert(isa(km.RotExpr, 'sym'), 'RotExpr must be sym.');

% Self-consistency: TSym(1:3,4) == PosExpr, TSym(1:3,1:3) == RotExpr
assert(isequal(km.TSym(1:3,4), km.PosExpr), 'TSym(1:3,4) ≠ PosExpr.');
assert(isequal(km.TSym(1:3,1:3), km.RotExpr), 'TSym(1:3,1:3) ≠ RotExpr.');
...
T_num = km.eval(vMap);
p_num = km.evalPos(vMap);
R_num = km.evalRot(vMap);
